...
namespace DX
{
            ...
            for (;; )
            {
                ...
                else if (0 == wcscmp(strCommand.c_str(), L"f"))
                {
                    ...
                    for (;;)
                    {
                        ...
                        vertex.position = positions[vertexIndex];//???? vertex..
    ...
    #pragma warning( suppress : 4127 )
        ...
        struct alignas(16) Material
        {
            ...
            bool bEmissive;
            //추가
            bool bUseTexture = false;// 텍스처 사용 여부
            ...
            vAmbient(0.2f, 0.2f, 0.2f),
                ...
                bSpecular(false),
                bEmissive(false),
                bUseTexture(false), // 기본값을 false로 설정
                strName{},
                strTexture{},
                strNormalTexture{},
                strSpecularTexture{},
            ...
            struct alignas(16) Material {
            DirectX::XMFLOAT3 vAmbient; // 12바이트
            float padding1;            // 4바이트 패딩
            DirectX::XMFLOAT3 vDiffuse; // 12바이트
            float padding2;            // 4바이트 패딩
            DirectX::XMFLOAT3 vSpecular; // 12바이트
            float padding3;            // 4바이트 패딩
            DirectX::XMFLOAT3 vEmissive; // 12바이트
            float padding4;            // 4바이트 패딩

            uint32_t nShininess;       // 4바이트 값이상
            float fAlpha;              // 4바이트
            int bSpecular;             // 4바이트 값이상
            int bEmissive;             // 4바이트

            int bUseTexture;           // 4바이트
            int padding5;              // 4바이트 패딩

            wchar_t strName[MAX_PATH];           // 문자열 데이터
            wchar_t strTexture[MAX_PATH];
            wchar_t strNormalTexture[MAX_PATH];
            wchar_t strSpecularTexture[MAX_PATH];
            wchar_t strEmissiveTexture[MAX_PATH];
            ...
            Material() noexcept :
                ...
                bSpecular(0),
                bEmissive(0),
                bUseTexture(0),//0 <<텍스쳐 사용안함 1<<사용함
                strName{},
                strTexture{},
                strNormalTexture{},
                strSpecularTexture{},
    ...
    private:
        ...
        public:
            ...
            std::vector<uint32_t> getAttributes() {
                return attributes;
            }
